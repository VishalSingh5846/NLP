...
class FixedWidth(Dataset):
    def __init__(self,df,word2VecObj,dic):
        self.df = df
        ...
        for idx in range(len(df)):
            ...
            feat = getMyFeatures(self.df['Headline'].iloc[idx],self.df['Body'].iloc[idx])
            
            label = self.df['Label'].iloc[idx]
            head =  nltk.word_tokenize(self.df['Headline'].iloc[idx])[:HEAD_WORDS]
            body =  nltk.word_tokenize(self.df['Body'].iloc[idx])[:BODY_WORDS]
...
class SimpleNN(nn.Module):
    def __init__(self,input_size, hidden_size, num_layers , output_size, bidirectional ,datasetLoaderTrain,datasetLoaderTest):
        ...
        global HEAD_WORDS
        global BODY_WORDS
        self.LayerO = nn.Linear( 234  , hidden_size)
...
class LSTMNetwork(nn.Module):
    def __init__(self,input_size, hidden_size, num_layers , output_size, bidirectional ,datasetLoaderTrain,datasetLoaderTest):
        ...
        self.LayerFinal = nn.Linear(output_size + hidden_size*2 + 234, output_size)
...
class RNNwithAttention(nn.Module):
    def __init__(self,input_size, hidden_size, num_layers , output_size, bidirectional ,datasetLoaderTrain,datasetLoaderTest):
        ...
        self.LayerFinal = nn.Linear(output_size + hidden_size*4 + 234, output_size)
        # self.Layer3 = nn.Dropout(self.dropout_p)
    ...
    def forward(self, head,body ,otherFeat):
        ...
        inp = torch.cat([inpH[-1],output,inpX,otherFeat],dim=1)
...
def runForFNC():
    global word2VecObjX
    dfTrain = readFNCData(DATA_PATH_FNC,'train')[:500]
    dfTest = readFNCData(DATA_PATH_FNC,'competition_test')[:200]
    # dfTest = readFNCData(DATA_PATH_FNC,'train')[500:700]
    tfB = None
    # print "Starting TF-IDF"
    # tfH = TfidfVectorizer(max_features=3000)
    # tfH.fit(dfTrain['Headline'].values)
    # tfB = TfidfVectorizer(max_features=300)
    # tfB.fit(dfTrain['Body'].values)
    # print "TF-IDF Complete!"
    dic = getMostCommonWordsDic(dfTrain)
    FNCDataTrain = FixedWidth(dfTrain,word2VecObjX,dic)
    FNCDataTest = FixedWidth(dfTest,word2VecObjX,dic)
    # FNCDataTrain = Cosine(dfTrain,word2VecObjX,tfB,tfB)
    # FNCDataTest = Cosine(dfTest,word2VecObjX,tfB,tfB)
    
    datasetLoaderTrain = torch.utils.data.DataLoader(FNCDataTrain, batch_size=10, shuffle=False, num_workers=4)
    datasetLoaderTest = torch.utils.data.DataLoader(FNCDataTest, batch_size=10, shuffle=False, num_workers=4)
